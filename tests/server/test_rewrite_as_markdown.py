...
from mcp_server import rewrite_as_markdown, mcp
...
def test_happy_path_returns_single_user_message(temp_docs_dir):
    ...
    messages = rewrite_as_markdown("notes.md")
...
def test_missing_file_returns_helpful_user_message(temp_docs_dir):
    messages = rewrite_as_markdown("missing.md")
...
def test_binary_check_runs_before_exists_check(temp_docs_dir):
    ...
    with pytest.raises(ValueError, match="is a binary format"):
        rewrite_as_markdown("missing.docx")


def test_binary_check_is_case_insensitive(temp_docs_dir):
    with pytest.raises(ValueError, match="is a binary format"):
        rewrite_as_markdown("FILE.PDF")
...
async def test_prompt_registration_metadata():
    ...
    matches = [p for p in prompts if p.name == "rewrite-as-markdown"]
    assert len(matches) == 1
    assert matches[0].description == "Rewrites the contents of a document in Markdown format."


@pytest.mark.anyio
async def test_prompt_argument_metadata():
    prompts = await mcp.list_prompts()
    prompt = next(p for p in prompts if p.name == "rewrite-as-markdown")
    ...
    assert filename_arg.description == "Filename of the document to rewrite."
